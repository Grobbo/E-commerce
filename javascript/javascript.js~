...
function itemize_list(list){
	...
	for(index = 0;index < list.length; ++index){
		string += '<div class="item_text"> <p>' + list[index].manufacturer+ '</div>' +
		...
		build_add_to_cart_button(list[index].id)+ '</div>'; 
	}
	return string;		
}
...
function build_add_to_cart_button (id){
	return '<div class="item_text"><form action="php/cart.php" method="POST"><input name="product_id" type="hidden" value="' + id +'"/><button class="add_button" type="submit">Add to Cart</button></form></div><br>';
}
